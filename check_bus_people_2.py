...
if DEBUG:
    cap = cv2.VideoCapture('./input/test1/test_1.avi')
else:
...
line_counter = LineZone(start=config.LINE_START2, end=config.LINE_END2)
...
def check_crossed(frame):
    ...
    yolo_det = filter_boxes_near_door(yolo_det, 150, 780)

    yolo_det_scaled = downscale_boxes(yolo_det, x_scale=0.3, y_scale=0.3)
    ...
    line_counter.trigger(tracks=trackableObjects, mode='camera2')
...
door_det_cls = Yolo_detections(model='door2.pt')
